...
def newsletter():
    ...
    try:
        with closing(get_db()) as db:
            ...
            # Insert the email (or ignore if already exists)
            db.execute('''
            ...
            with closing(get_db()) as db:
                db.execute(
                    ...
                    (
                        ...
                        1  # Set product as active by default
                    )
                )
                db.commit()
...
def delete_product(product_id):
    with closing(get_db()) as db:
        ...
        if result.rowcount > 0:
            ...
            db.execute('''
                INSERT INTO product_logs (product_id, action, performed_by, timestamp)
                VALUES (?, ?, ?, ?)
            ''', (product_id, 'deactivated', session['username'], datetime.now()))
            db.commit()
...
def checkout():
    ...
    with closing(get_db()) as db:
        ...
        # Get cart items with details from database
        cart_items = []
        total = 0
    ...
    try:
        with closing(get_db()) as db:
            ...
            order_id = db.execute(
                ...
                (session['user_id'], address['id'], 'processing', request.json.get('total'))
            ).lastrowid
